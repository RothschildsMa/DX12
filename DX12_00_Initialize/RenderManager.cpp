...
*/
	...
	//パイプライン生成
	{
		...
		m_PipelineState["Unlit"] = CreatePipeline("Shader\\UnlitVS.cso", "Shader\\UnlitPS.cso", RTVFormats, _countof(RTVFormats));
	...
	//ジオメトリパイプライン生成
	{
		...
		m_PipelineState["Geometry"] = CreatePipeline("Shader\\GeometryVS.cso", "Shader\\GeometryPS.cso", RTVFormats, _countof(RTVFormats));
	...
	//デファードパイプライン生成
	{
		...
		m_PipelineState["Deferred"] = CreatePipeline("Shader\\DeferredVS.cso", "Shader\\DeferredPS.cso", RTVFormats, _countof(RTVFormats));
...
void RenderManager::DrawEnd()
{
	...
	m_GraphicsCommandList->OMSetRenderTargets(1, 
		&m_RenderTargetHandle[m_RTIndex], TRUE, 
		&m_DepthBufferHandle);
	...
	//ImGUI
	{
		...
		//colorbuffer debug
		{
			...
			ImGui::Image((void*)m_ColorBuffer->SRVHandle.ptr, ImVec2(320.0f, 180.0f));

			ImGui::Text("NormalBuffer");
			ImGui::Image((void*)m_NormalBuffer->SRVHandle.ptr, ImVec2(320.0f, 180.0f));

			ImGui::Text("PositionBuffer");
			ImGui::Image((void*)m_PositionBuffer->SRVHandle.ptr, ImVec2(320.0f, 180.0f));
